...
- The task of identifying numbers can be broken down into a separate task for each layer, then built up from each layer to the text
  - For example, the first layer represents the pixel input, and the next and signal a line segment that the pixels have lit up, and the next can be a whole bunch of segments together that make up a number
...
- Each neuron in one layer has a connection to each of the neuron the next layer
